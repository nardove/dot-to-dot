...
export default class SketchControls extends Component {
	...
	componentDidMount() {
		// Initial button size is set by offButton()
	...
	handleUndoDot() {
		...
		TweenMax.to(this.undoBtnRef, 0.1, {
			opacity: 1,
			scale: 1,
			ease: Power2.easeOut,
			repeat: 1,
		...
		return (
			<div className='sketch-controls container fixed-bottom'>
				...
				<img
					...
					ref={img => (this.undoBtnRef = img)}
				/>
				<img
					className='img-btn'
					src={pathLineIcon}
					alt='Show/Hide path line'
					title='Show/Hide path line'
					onClick={this.togglePathVisiblility}
					ref={img => (this.pathBtnRef = img)}
				/>
				<img
					className='img-btn'
					src={paletteIcon}
					alt='Dots color options'
